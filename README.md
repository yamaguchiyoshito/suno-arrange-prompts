...
- アーティストやグループを指定した着想案は `artist` タグ、プロデューサーを指定した着想案は `producer` タグ、レーベルを指定した着想案は `source` タグで保持し、GitHub Pagesから着想元単位で絞り込めます。
...
- `group: artist` は着想元アーティスト・グループの任意タグです。`group_label: 着想元アーティスト` を指定します。既存のプロデューサータグは `group: producer` のまま保持します。
